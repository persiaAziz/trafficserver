...
int SNIConfigParams::getEnum(char* actionTag) {
...
void SNIConfigParams::setPropertyConfig(int id, char* servername, void* param)
{
...
NextHopProperty* SNIConfigParams::getPropertyConfig(cchar* servername) const
{
...
char* SNIConfigParams::ParseSNIConfigLine(matcher_line const& line_info, ActionItem*& ai) {
...
actionVector* SNIConfigParams::get(cchar* servername) const{
...
SNIConfigParams::printSNImap() const
{
  ...
  {
    Debug("ssl", "Domain name in the map %s: # of registered action items %d",(char*) keys.get(i),sni_action_map.get(keys.get(i))->size());
  }
}
...
void SNIConfigParams::cleanup()
{
    Vec<cchar*> keys;
    sni_action_map.get_keys(keys);
    for (size_t i = keys.length()-1; i >=0 ; i--)
    {
