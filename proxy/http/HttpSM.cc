...
extern std::unordered_map<std::string, std::string> TunnelMap; // stores the name of the servers to tunnel to
...
HttpSM::setup_blind_tunnel_port()
{
  ...
  if (ssl_vc && ssl_vc->GetSNIMapping()) {
    if (!t_state.hdr_info.client_request.url_get()->host_get(&host_len)) {
      ...
      std::unordered_map<std::string, std::string>::const_iterator got = TunnelMap.find(std::string(ssl_vc->serverName));

      if (got != TunnelMap.end()) {
        ts::string_view addr, port;
        if (ats_ip_parse(ts::string_view(got->second), &addr, &port) == 0) {
          t_state.hdr_info.client_request.url_get()->host_set(addr.data(), addr.length());
          if (port.length() > 0) {
            t_state.hdr_info.client_request.url_get()->port_set(atoi(port.data()));
          } else {
