    ...
    err(false)
{
  ...
  if (Config::getSingleton().loadConfig(conf)) {
    Config::getSingleton().getValue("subconfig", "SubRedisConnectTimeout", m_redisConnectTimeout);
    Config::getSingleton().getValue("subconfig", "SubRedisRetryDelay", m_redisRetryDelay);
    Config::getSingleton().getValue("subconfig", "SubColoChannel", m_channel);
    Config::getSingleton().getValue("subconfig", "SubRedisEndpoints", redisEndpointsStr);
  }
  ...
  if (!(get_redis_auth_key(redis_auth_key, MAX_REDIS_KEYSIZE))) {
    err = true;
    TSError("RedisPublisher::RedisPublisher.Cannot get redis AUTH password from ykeydb.");
    redis_passwd.clear();
  } else {
