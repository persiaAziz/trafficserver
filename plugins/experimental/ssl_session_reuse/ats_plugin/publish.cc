    ...
    err(false)
{
  if (Config::getSingleton().loadConfig(conf)) {
    ...
    Config::getSingleton().getValue("pubconfig", "PubRedisEndpoints", m_redisEndpointsStr);
    Config::getSingleton().getValue("pubconfig", "PubRedisConnectTimeout", m_redisConnectTimeout);
    ...
    Config::getSingleton().getValue("pubconfig", "PubRedisRetryDelay", m_redisRetryDelay);
    Config::getSingleton().getValue("pubconfig", "PubMaxQueuedMessages", m_maxQueuedMessages);
    Config::getSingleton().getValue("pubconfig", "PoolRedisConnectTimeout", m_poolRedisConnectTimeout);
    Config::getSingleton().getValue("pubconfig", "GetSessionTimeout", m_getSessionTimeout);
    Config::getSingleton().getValue("ssl_session", "SessionLifetime", m_sessionLifetime);
    Config::getSingleton().getValue("ssl_session", "SlowlogThresholdTime", m_slowlogThresholdTime);
    Config::getSingleton().getValue("ssl_session", "EnableRedisStore", m_enableRedisStore);
  }
  ...
  TSDebug(PLUGIN, "RedisPublisher::RedisPublisher.PoolRedisConnectTimeout= %d GetSessionTimeout= %d", m_poolRedisConnectTimeout,
          m_getSessionTimeout);
...
RedisPublisher::get_session(const std::string &channel)
{
  ...
  for (uint32_t i = 0; i < m_redisEndpoints.size(); i++) {
    ...
    uint64_t time = (now.tv_sec * 1000 + now.tv_usec / 1000) - (start.tv_sec * 1000 + start.tv_usec / 1000);
    TSDebug(PLUGIN, "RedisPublisher::get_session. The time consumed: %llu ms", static_cast<long long unsigned>(time));
    if (time > m_getSessionTimeout) {
      TSError("RedisPublisher::get_session. Time out when trying to get a redis connection!");
      return ret;
    }
  ...
  for (uint32_t i = 0; i < m_redisEndpoints.size(); i++) {
    ...
    if (conn) {
      reply = static_cast<redisReply *>(
        redisCommand(conn->c_ptr(), "SET %s %s EX %d", msg.channel.c_str(), msg.data.c_str(), m_sessionLifetime));
